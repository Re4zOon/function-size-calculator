...
class TestCSharpParser:
    """Tests for CSharpParser."""

    @pytest.fixture(autouse=True)
    def _setup(self, fixtures_dir: Path):
        self.cs_file = fixtures_dir / "Sample.cs"

    def test_parse_csharp_file(self):
        functions = CSharpParser.parse_functions(str(self.cs_file))

        assert len(functions) > 0

        func_names = [f.name for f in functions]
        assert "simpleAdd" in func_names
        assert "largeMethod" in func_names
        assert "asyncMethod" in func_names
        assert "virtualMethod" in func_names
        assert "ToString" in func_names
...
class TestPythonParser:
    ...
    def test_python_class_methods(self):
        ...
        assert "instance_method" in func_names
    ...
    def test_multiline_signature(self, tmp_path: Path):
        temp_file = tmp_path / "temp_multiline.py"
        temp_file.write_text(
            """def multi_line_func(
    arg1: str,
    arg2: int,
...
"""
        ...
        functions = PythonParser.parse_functions(str(temp_file))

        assert len(functions) == 1
        assert functions[0].name == "multi_line_func"
        assert functions[0].size == 8
